...
/* Define the duration of the long press in 3000 ms */
const uint32_t ui32_LONG_PRESS_TIME = 3000;
...
int main(void)
{
    ...
    while(1){
        ...
        pressDuration = releasedTime - pressedTime; /* Maybe move this operation to avoid subtract to 0  if no released time is set */

        if(pressDuration > ui32_LONG_PRESS_TIME && pressDuration < 3500)
            GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_1, 0x02); /* Check if pressDuration should be reset because it continuously enter this statement */
